...
const Cart = () => {
  ...
  return (
    <div className="flex gap-2">
      <Navbar />
      <div className="mt-24 w-full overflow-y-scroll h-[90vh] ">
        {!cart?.length && (
          <div className="w-[90%] h-[24rem] flex items-center justify-center text-center border rounded-lg m-4">
            {" "}
            Cart Empty
          </div>
        )}
        {cart?.map((cartValue) => (
          <div className="flex flex-col gap-2 border p-2 m-4 rounded-md hover:bg-gray-50 transition ease-in-out duration-150">
            <div className="flex justify-between " key={cartValue.product._id}>
              <div
                onClick={() => navigate(`/product/${cartValue.product._id}`)}
      ...
      <div className="w-full flex flex-col gap-2 mt-24 mx-4">
        ...
        <div className="m-4 flex gap-2">
          <button
            ...
            onClick={() => navigate("/confirmCheckout", { state: { address } })}
